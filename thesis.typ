...
In words, the linear approximation of an #Lsmooth function around some reference point is controlled by a quadratic in the distance from that point. This lemma will plays a central role in the construction of the gradient descent algorithm in the following section.

The final important notion required to precisely state the setting of #cite(<ryu2026>, form: "prose") is that of a _first-order_ algorithm for the purposes of this thesis.

#definition([First-order algorithm #cite(<nesterov2018lectures>, supplement: "Section 1.1.2")])[
  A first-order algorithm is an interative procedure that produces a sequence $seqfull({x_k}, k >=0) subset.eq RR^p$ where
  $
    x_0 in RR^p, space x_(k+1) = g(seqfull({x_k, f(x_k), nabla f(x_k)}, k>=0)).
  $
  for some function $g : (RR^p times RR times RR^p) arrow.r RR^p$.
]
...
algorithms in this these are any process by which a sequence $x_1, x_2, ... subset RR^p$ is generated.
...
Formally, I consider the optimisation problem

$
 minimise_(x in RR^p) f(x),
$<minfx>

where $f : RR^n arrow.r RR$ is #Lsmooth and convex, and consider iterative procedures producing a sequence $seqfull({x_n},n>=0)$ such that

$
  f(x_k) arrow.long.r_(k arrow.r infinity) f_star = inf_(x in RR^p) f(x).
$

The algorithms under consideration in this thesis are only 'first-order' algorithms.
...
For a discussion of algorithms to be relevant, I assume throughout that at least one solution to the problem (@minfx) exists, with the potential for an infinity of solutions, depending on the particular function $f$. In the smooth and convex setting, the existence of solutions is often easy to establish, particularly if $dom(f)$ is restricted to a convex and compact subset of $RR^n$.
